    ...
    <div class="max-w-sm flex flex-col items-start p-6 bg-white border border-zinc-200 rounded-lg shadow dark:bg-zinc-800 dark:border-zinc-700 transition duration-300 hover:scale-105">
        ...
        <div v-else-if="set.visibility !== undefined" class="text-sm text-zinc-500 dark:text-zinc-400 mb-3" :title="resolvedCreationTime.toLocaleString()">
            ...
            <span
                ...
                <span class="w-2 h-2 mr-1 rounded-full" :class="visibilityBadgeColors[getVisibility(set.visibility)][1]"></span>
        ...
        <div class="flex flex-row flex-wrap gap-2" v-if="showEditControls">
            ...
            <button @click="$emit('delete-set')" class="mt-auto inline-flex items-center px-3 py-2 text-sm font-medium text-center transition-colors text-red-500 bg-red-100 hover:bg-red-500 hover:text-white rounded-lg focus:ring-4 focus:outline-none focus:ring-red-500">
                Delete
...
const props = defineProps<{
    set: Pick<VocabSet, "name" | "id" | "collections" | "numTerms" | "likes" | "id" | "createTime"> & Partial<Pick<VocabSet, "visibility" | "creationTime">>
    creator?: UserProfile,
