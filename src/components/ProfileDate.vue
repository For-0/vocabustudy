    ...
    <div class="flex items-center space-x-2">
        ...
        <div class="font-medium dark:text-white max-w-full">
            <div class="flex min-w-0">
                <router-link v-if="profile.uid" class="min-w-0 truncate" :to="{ name: 'user-profile', params: { uid: profile.uid } }">{{ profile.displayName }}</router-link>
                <span v-else class="min-w-0 truncate">{{ profile.displayName }}</span>
