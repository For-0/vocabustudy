<template>
    <div v-if="images.length > 0" class="relative w-full" :class="{ 'lg:h-full lg:w-auto': !alwaysVertical }">
        <!-- Image -->
        <img :src="images[currentImage]" alt="Set image" class="block rounded-lg max-h-96 w-full object-contain" :class="{ 'lg:max-w-2xl lg:h-full lg:min-w-[16rem] lg:max-h-none': !alwaysVertical }">
        <!-- Slider indicators -->
